    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-xl shadow-2xl border border-slate-200 max-w-2xl w-full overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          {!isLoading && response?.success && (
            <div className="py-2 space-y-6">
              ...
              {/* Primary Filled Custom Template Card (if configured/generated) */}
              {files.templateFilledUrl && (
                <div className="bg-gradient-to-r from-red-50 to-orange-50 border-2 border-[#D32F2F] rounded-xl p-4 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4 text-left">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-lg bg-[#D32F2F] text-white flex items-center justify-center shrink-0 shadow-md">
                      <FileText className="w-6 h-6 text-white" />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold uppercase tracking-wider bg-[#D32F2F] text-white px-2 py-0.5 rounded-full">
                          Master Template Output (Filled)
                        </span>
                      ...
                      <div className="text-sm font-bold text-slate-900 mt-1">
                        {response.customerName ? `${response.customerName} - Filled Master Template` : 'Populated Custom Template Document'}
                      </div>
                      <div className="text-[11px] text-slate-600 mt-0.5">
                        Cloned from your template with all customer, claim & milestone variables filled
                      </div>
                    </div>
                  </div>
                  <a
                    href={files.templateFilledUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full sm:w-auto px-4 py-2.5 bg-[#D32F2F] hover:bg-[#B71C1C] text-white text-xs font-bold rounded-lg shadow-sm transition-colors inline-flex items-center justify-center gap-2 shrink-0"
                  >
                    <FileText className="w-4 h-4 text-white" />
                    <span>Open Filled Template Doc</span>
              ...
              <div className="space-y-3 text-left">
                <div className="flex items-center justify-between">
                  <div className="text-xs font-bold uppercase tracking-wider text-slate-600 flex items-center gap-2">
                    ...
                    <span>Executive Restoration Documents</span>
                  </div>
                  {onOpenTemplateGuide && (
                    <button
                      type="button"
                      onClick={onOpenTemplateGuide}
                      className="text-[11px] font-semibold text-[#D32F2F] hover:underline inline-flex items-center gap-1"
                    >
                      <FileText className="w-3 h-3" />
                      <span>Template & Merge Tags Guide</span>
                    </button>
                  )}
                </div>
        ...
        <div className="bg-slate-50 px-6 py-4 border-t border-slate-200 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {!isLoading && response?.success && onCreateChangeOrder && (
              <button
                type="button"
                onClick={onCreateChangeOrder}
                className="px-3.5 py-2 text-xs font-bold text-slate-800 bg-white hover:bg-slate-100 border border-slate-300 rounded-lg shadow-xs transition-colors inline-flex items-center gap-1.5"
              >
                <FileCheck2 className="w-4 h-4 text-[#D32F2F]" />
